...
trait CanvasPaintTaskPrivateHelpers {
    // It reads pixel data from the canvas
    fn read_pixels(&self, Rect<i32>, Size2D<i32>) -> Vec<u8>;
    // It writes pixel data to the canvas
    fn write_pixels(&self, Vec<u8>, Size2D<i32>, Rect<i32>, Rect<i32>, bool);
    // Used by put_image_data to calculate the dirty area (where the pixels
    // are written in the destination canvas)
    // following the steps described on the spec:
    // https://html.spec.whatwg.org/#dom-context-2d-putimagedata
    fn calculate_dirty_rect(&self, Rect<i32>, Rect<i32>) -> Rect<i32>;
    // Used by draw_image to write pixels to the destination canvas
    fn write_image(&self, Vec<u8>, Rect<i32>, Rect<i32>, bool);
}

impl<'a> CanvasPaintTaskPrivateHelpers for CanvasPaintTask<'a> {
    ...
    fn calculate_dirty_rect(&self,
        ...
        // 3) If dirtyY is negative, let dirtyWidth be dirtyWidth+dirtyX, and let dirtyX be zero.
    ...
    // dest_rect: Ares of the destination canvas where the pixels will be copied
...
impl<'a> CanvasPaintTask<'a> {
    ...
    fn get_image_data(&self, mut dest_rect: Rect<i32>, canvas_size: Size2D<i32>, chan: Sender<Vec<u8>>) {
        ...
        // It reads image data from canvas
        let mut dest_data = self.read_pixels(dest_rect, canvas_size);

        // bgra -> rgba
        // It sends back the image data
    ...
    fn put_image_data(&mut self, mut imagedata: Vec<u8>,
        ...
        let source_rect = match dirty_rect {
            // It calculates the dirty area following the steps described in the spec:
            // https://html.spec.whatwg.org/#dom-context-2d-putimagedata
            Some(dirty_rect) =>
